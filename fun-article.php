...
// 文章新窗口打开开关--------------------------www.mysqil.com--------------------------
function kizumi_article_new_window() {
    return get_kizumi('kizumi_article_new_window_switch')?'target="_blank"':'';
}

// 开启所有文章形式支持--------------------------www.mysqil.com--------------------------
if(get_kizumi('kizumi_article_support_switch')){
...
// 缩略图尺寸设定--------------------------www.mysqil.com--------------------------
if(get_kizumi('kizumi_article_thumbnail_size_switch')){
function kizumi_article_thumbnail_size($size) {
    $width  = intval(get_kizumi('kizumi_article_thumbnail_width')) ?: 300; 
    $height = intval(get_kizumi('kizumi_article_thumbnail_height')) ?: 200;
    return array($width, $height); 
}
add_filter('post_thumbnail_size', 'kizumi_article_thumbnail_size');
}

// 文章缩略图逻辑--------------------------www.mysqil.com--------------------------
function kizumi_article_thumbnail_src() {
    global $post;
    $src='';
    if ($thumbnail_id = get_post_thumbnail_id()) {
        ...
        if(get_kizumi('kizumi_article_thumbnail_random_api')){
            $src=get_kizumi('kizumi_article_thumbnail_random_api_url');
        }else{
            ...
            if (!empty($random_images)) {
                ...
                $src = kizumi_theme_url().'/assets/images/default-thumbnail.jpg';
            }
        }
    }
    return $src ?: kizumi_theme_url().'/assets/images/default-thumbnail.jpg';
}

//文章点击数换算K--------------------------www.mysqil.com--------------------------
...
//自定义段长度--------------------------www.mysqil.com--------------------------
function custom_excerpt_length( $length ){
return 200;
}
add_filter( 'excerpt_length', 'custom_excerpt_length');

//文章、评论内容缩短--------------------------www.mysqil.com--------------------------
...
// 表格替换--------------------------www.mysqil.com--------------------------
function kizumi_table_replace($text){
	$replace = array( '<table>' => '<div class="table-responsive"><table class="table" >','</table>' => '</table></div>' );
	$text = str_replace(array_keys($replace), $replace, $text);
	return $text;}
add_filter('the_content', 'kizumi_table_replace');

//防止代码转义--------------------------www.mysqil.com--------------------------
function kizumi_prettify_esc_html($content){
    $regex = '/(<pre\s+[^>]*?class\s*?=\s*?[",\'].*?prettyprint.*?[",\'].*?>)(.*?)(<\/pre>)/sim';
    return preg_replace_callback($regex, 'kizumi_prettify_esc_callback', $content);}
function kizumi_prettify_esc_callback($matches){
    $tag_open = $matches[1];
    $content = $matches[2];
    $tag_close = $matches[3];
    $content = esc_html($content);
    return $tag_open . $content . $tag_close;}
add_filter('the_content', 'kizumi_prettify_esc_html', 2);
add_filter('comment_text', 'kizumi_prettify_esc_html', 2);

//强制兼容--------------------------www.mysqil.com--------------------------
function kizumi_prettify_replace($text){
	$replace = array( '<pre>' => '<pre class="prettyprint linenums" >','<pre class="prettyprint">' => '<pre class="prettyprint linenums" >' );
	$text = str_replace(array_keys($replace), $replace, $text);
	return $text;}
add_filter('the_content', 'kizumi_prettify_replace');

// 自动设置特色图片--------------------------www.mysqil.com--------------------------
...
// 自适应图片--------------------------www.mysqil.com--------------------------
function kizumi_remove_width_height($content) {
...
add_filter('the_content', 'kizumi_remove_width_height', 99);


// 图片懒加载--------------------------www.mysqil.com--------------------------
function kizumi_lazy_content_load_images($content) {
...
if(!is_admin()){
    add_filter('the_content', 'kizumi_lazy_content_load_images', 99);
}

// fancybox--------------------------www.mysqil.com--------------------------
function kizumi_fancybox_replace ($content) {
    global $post;
...
add_filter('the_content', 'kizumi_fancybox_replace', 99);
...
// 分页导航函数--------------------------www.mysqil.com--------------------------
if ( ! function_exists( 'kizumi_pagination' ) ) :
function kizumi_pagination($query = null) {
    $paging_type = get_kizumi('kizumi_article_paging_type');
    if($paging_type == 'multi'){
        ...
        if ($current > 1) {
            ...
            previous_posts_link('<span class="page-link"><i class="fa fa-arrow-left"></i> '.__('上一页', 'kizumi').'</span>');
            echo '</li>';
        }
        if ($current < $total) {
            echo '<li class="page-item ms-2">';
            next_posts_link('<span class="page-link">'.__('下一页', 'kizumi').' <i class="fa fa-arrow-right"></i></span>', $total);
            echo '</li>';
        }
        echo '</ul></nav>';
...
function p_link( $i, $title = '', $w='' ) {
    if ( $title == '' ) $title = __('页', 'kizumi-com')." {$i}";
    $itext = $i;
    if( $i == 0 ){
        $itext = __('<i class="fa fa-angle-double-left"></i>', 'kizumi-com');
    }
    if( $w ){
        $itext = __('<i class="fa fa-angle-double-right"></i>', 'kizumi-com');
    }
...
add_action('wp_ajax_post_like', 'kizumi_post_like');
add_action('wp_ajax_nopriv_post_like', 'kizumi_post_like');
...
add_action('wp_ajax_post_favorite', 'kizumi_post_favorite');

// 处理删除收藏
function kizumi_delete_favorite() {
    if (!is_user_logged_in()) {
        wp_send_json_error(['message' => '请先登录']);
        return;
    }
...
add_action('wp_ajax_delete_favorite', 'kizumi_delete_favorite');
