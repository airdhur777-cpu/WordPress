...
function kizumi_comment($comment, $args = array(), $depth = 1) {
    ...
    <div id="comment-<?php comment_ID(); ?>" class="comment-item <?php echo $depth > 1 ? 'child' : 'parent'; ?>">
            ...
            <img src="<?php echo kizumi_lazy_load_images(); ?>" data-src="<?php echo kizumi_get_avatar_url($comment->comment_author_email, 60); ?>" alt="评论头像" class="lazy">
            </div>
        <div class="comment-content">
            <div class="comment-meta">
                ...
                <?php if (user_can($comment->user_id, 'administrator')): ?>
                    <span class="comment-badge"><?php echo get_kizumi('kizumi_comment_blogger_tag')?get_kizumi('kizumi_comment_blogger_tag'):'博主'; ?></span>
                <?php endif; ?>
...
function kizumi_comment_add_at($comment_text, $comment) {  
...
add_filter('comment_text', 'kizumi_comment_add_at', 10, 2);
...
function ajax_comment_callback() {
    ...
    $comment_content = $comment_data['comment'];
    if(get_kizumi('kizumi_comment_english_switch')){
    ...
    if ($commentarr['comment_parent'] > 0) {
        ...
        if ($parent_comment && $parent_comment->comment_author_email) {
            kizumi_comment_reply_notification($comment_id);
        }
    }
    }
    if (get_kizumi('kizumi_smtp_mail_switch') && get_kizumi('kizumi_new_comment_notice_switch')) {
        kizumi_new_comment_notice_email($comment_id);
    }

    if (get_kizumi('kizumi_robot_notice_switch') && get_kizumi('kizumi_new_comment_notice_robot_switch')) {
        kizumi_robot_msg_comment($comment_id);
    }

    $comment = get_comment($comment_id);
    ob_start();
    kizumi_comment($comment, array('max_depth' => 1), 1);
...
// 添加后台评论回复的邮件通知
function kizumi_admin_comment_reply($comment_id, $comment_object) {
    if (!get_kizumi('kizumi_smtp_mail_switch')) {
        return;
    }
...
add_action('wp_insert_comment', 'kizumi_admin_comment_reply', 10, 2);
remove_action('comment_post', 'kizumi_comment_reply_notification');
