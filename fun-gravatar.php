...
// Gravatar头像--------------------------www.mysqil.com--------------------------
function kizumi_getavatar_host() {
    $gravatar_Url = 'cravatar.cn/avatar';

    switch (get_kizumi('kizumi_gravatar_url')) {
        case 'cn':
...
function kizumi_qqavatar_host() {
    $qqavatar_Url = 'q2.qlogo.cn';
    switch (get_kizumi('kizumi_qqavatar_url')) {
        case 'Q1':
...
function kizumi_get_avatar($avatar, $id_or_email, $size = 96, $default = '', $alt = '', $args = array()) {
    $email = '';
    $user_id = '';
    ...
    $gavatarurl = 'https://' . kizumi_getavatar_host() . '/' . $hash;
    ...
    if (isset($user_id)) {
        ...
        if ($user_avatar_url) { 
            ...
            if (preg_match("/^\d+$/", $qq)) {
                $qqavatar = "https://" . kizumi_qqavatar_host() . "/headimg_dl?dst_uin=" . $qq . "&spec=100";
...
add_filter('get_avatar', 'kizumi_get_avatar', 10, 6);

// 提取头像地址--------------------------www.mysqil.com--------------------------
function kizumi_get_avatar_url($id_or_email, $size = 100) {
    $email = '';
    $user_id = '';
        if (is_numeric($id_or_email)) {
    ...
    if (stripos($email, "@qq.com")) {
        $qq = str_ireplace("@qq.com", "", $email);
        if (preg_match("/^\d+$/", $qq)) {
            return "https://" . kizumi_qqavatar_host() . "/headimg_dl?dst_uin=" . $qq . "&spec=100";
        }
    }
    $hash = md5(strtolower(trim($email)));
    return 'https://' . kizumi_getavatar_host() . '/' . $hash;
}
