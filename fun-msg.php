...
//发件邮件统一模板
function kizumi_smtp_mail_template($to, $subject, $message) {
    if (!is_email($to)) {
    ...
    $from_email = get_option('kizumi_smtp_from');
...
//新用户注册消息通知
function kizumi_new_user_register($user_id){
    ...
    kizumi_smtp_mail_template($admin_email, $subject, $message);
}

//评论消息通知
function kizumi_comment_notification($comment_id){
    ...
    kizumi_smtp_mail_template($post->post_author, $subject, $message);
}
if(get_kizumi('kizumi_new_comment_notice_switch')){
    add_action('comment_post', 'kizumi_comment_notification');
}

//评论回复消息通知
function kizumi_comment_reply_notification($comment_id) {
    $comment = get_comment($comment_id);   
    // 基础检查
    ...
    kizumi_smtp_mail_template($parent_comment->comment_author_email, $subject, $message);
}


//找回密码邮件
function kizumi_reset_password_email($user_login) {
    // 获取用户信息
    ...
    return kizumi_smtp_mail_template($user_email, $subject, $message);
}

//会员注册成功发生邮件
function kizumi_new_user_register_email($user_id){
    $user = get_user_by('id', $user_id);
    ...
    kizumi_smtp_mail_template($user->user_email, $subject, $message);
}
//add_action('user_register', 'kizumi_new_user_register_email');  

//验证码注册模板
function kizumi_verification_code_register_email($email, $verification_code = ''){
    ...
    return kizumi_smtp_mail_template($email, $subject, $message);
}




if(get_kizumi('kizumi_robot_notice_switch')){
//机器人post接口消息统一模板
function kizumi_robot_post_template($remote_server, $post_string) {  
    $ch = curl_init();  
...
//评论机器人通知
function kizumi_robot_msg_comment($comment_id){
    $comment = get_comment($comment_id);
    ...
    $message = $text . "\n" . "作者: $comment->comment_author \n邮箱: $comment->comment_author_email \n评论: $comment->comment_content \n 点击查看：$siteurl/?p=$comment->comment_post_ID#comments";
		if(get_kizumi('kizumi_robot_channel') == 'qq_group' ){			
			$msgid	=	get_kizumi('kizumi_robot_msg_user');
            $apiurl	=	get_kizumi('kizumi_robot_api_url').'/send_private_msg?group_id='.$msgid;
            $data = array ('msgtype' => 'text','text' => array ('content' => $message));
			$data_string = json_encode($data);
			return $result = kizumi_robot_post_template($apiurl, $data_string);  
			}
        if(get_kizumi('kizumi_robot_channel') == 'qq_user' ){			
			$msgid	=	get_kizumi('kizumi_robot_msg_user');
            $apiurl	=	get_kizumi('kizumi_robot_api_url').'/send_private_msg?user_id='.$msgid;
            $data = array ('msgtype' => 'text','text' => array ('content' => $message));
			$data_string = json_encode($data);
			return $result = kizumi_robot_post_template($apiurl, $data_string);  
			}    
		if(get_kizumi('kizumi_robot_channel') == 'dingtalk' ){
			$time    = intval(microtime(true) * 1000);
			$secret  = get_kizumi('kizumi_robot_api_key');
			$sign    = urlencode(base64_encode(hash_hmac('sha256', "{$time}\n{$secret}", $secret, true)));
			$apiurl	=	get_kizumi('kizumi_robot_api_url').'&timestamp='.$time.'&sign='.$sign;
			$data = array ('msgtype' => 'text','text' => array ('content' => $message));
			$data_string = json_encode($data);
			return $result = kizumi_robot_post_template($apiurl, $data_string);  
			}
        if(get_kizumi('kizumi_robot_channel') == 'telegram' ){
            $msgid	=	get_kizumi('kizumi_robot_msg_user');
            $key_token = get_kizumi('kizumi_robot_api_key');
            $apiurl = 'https://api.telegram.org/bot'.$key_token.'/sendMessage';
            $postdata = http_build_query(array('chat_id' => $msgid,'text' => $message));
            return $result = kizumi_robot_post_template($apiurl, $postdata);  
            }    
	}
    if(get_kizumi('kizumi_new_comment_notice_robot_switch')){
        add_action('comment_post', 'kizumi_robot_msg_comment');
    }

//新注册会员机器人通知	
function kizumi_robot_msg_reguser($user_id='',$user_email=''){	
...
if(get_kizumi('kizumi_new_user_register_notice_robot_switch')){
    add_action('user_register', 'kizumi_robot_msg_reguser');
}
//机器人开关
}
