...
// 头像上传处理
add_action('wp_ajax_upload_avatar', 'kizumi_upload_avatar');

function kizumi_upload_avatar() {
    if (!is_user_logged_in()) {
        wp_send_json_error(['message' => '请先登录']);
        return;
    }
    $nonce = $_POST['nonce'];
    if (!wp_verify_nonce($nonce, 'kizumi_ajax_nonce')) {
...
// 用户信息更新处理
add_action('wp_ajax_update_user_profile', 'kizumi_update_user_profile');

function kizumi_update_user_profile() {
    if (!is_user_logged_in()) {
        wp_send_json_error(['message' => '请先登录']);
        return;
    }
...
// 用户密码更新处理
add_action('wp_ajax_update_user_password', 'kizumi_update_user_password');

function kizumi_update_user_password() {
    if (!is_user_logged_in()) {
        wp_send_json_error(['message' => '请先登录']);
        return;
    }
...
add_action('wp_ajax_kizumi_form_money_card', 'kizumi_form_money_card');
add_action('wp_ajax_nopriv_kizumi_form_money_card', 'kizumi_form_money_card');

function kizumi_form_money_card() {
...
add_action('wp_ajax_kizumi_form_money_online', 'kizumi_form_money_online');
add_action('wp_ajax_nopriv_kizumi_form_money_online', 'kizumi_form_money_online');

function kizumi_form_money_online() {
