...
// 用户中心链接设置--------------------------www.mysqil.com--------------------------
function kizumi_user_center_link_page(){
    $kizumi_user_center_link_page = get_kizumi('kizumi_user_center_link_page');
    if($kizumi_user_center_link_page){
        return get_the_permalink($kizumi_user_center_link_page);
    }else{
        return false;
    }
}

// 注册页面链接设置--------------------------www.mysqil.com--------------------------
function kizumi_sign_up_link_page(){
    $kizumi_sign_up_link_page = get_kizumi('kizumi_sign_up_link_page');
    if($kizumi_sign_up_link_page){
        return get_the_permalink($kizumi_sign_up_link_page);
    }else{
        return false;
    }
}
...
// 登录页面链接设置--------------------------www.mysqil.com--------------------------
function kizumi_sign_in_link_page(){
     $kizumi_sign_in_link_page = get_kizumi('kizumi_sign_in_link_page');
    if($kizumi_sign_in_link_page){
        return get_the_permalink($kizumi_sign_in_link_page);
    }else{
        return false;
    }
}

// 重置密码页面链接设置--------------------------www.mysqil.com--------------------------
function kizumi_reset_password_link_page(){
    $kizumi_reset_password_link_page = get_kizumi('kizumi_reset_password_link_page');
    if($kizumi_reset_password_link_page){
        return get_the_permalink($kizumi_reset_password_link_page);
    }else{
        return false;
    }
}

// 充值卡购买链接设置--------------------------www.mysqil.com--------------------------
function kizumi_czcard_src(){
    $kizumi_czcard_src = get_kizumi('kizumi_czcard_src');
    if($kizumi_czcard_src){
        return $kizumi_czcard_src;
    }else{
        return false;
    }
}
...
function handle_user_signup() {
    ...
    if(get_kizumi('kizumi_smtp_mail_switch')){   
        if(get_kizumi('kizumi_new_user_register_notice_switch')){
            kizumi_new_user_register($user_id);
        }
    }
    if(get_kizumi('kizumi_robot_notice_switch')){
        if(get_kizumi('kizumi_new_user_register_notice_robot_switch')){
            kizumi_robot_msg_reguser($user_id,$user->user_email);
        }
    } 
    delete_transient('verification_code_' . $formData['email']);  
    kizumi_new_user_register_email($user_id);
...
function kizumi_allow_chinese_username($username, $raw_username, $strict) {
    if (!$strict) {
        return $username;
    } 
...
add_filter('sanitize_user', 'kizumi_allow_chinese_username', 10, 3);
...
function handle_send_verification_code() {
    ...
    if (kizumi_verification_code_register_email($email, $verification_code)) {
...
function handle_reset_password_request() {
    ...
    if(kizumi_reset_password_email($user->user_login)){
...
// 处理用户注册时间
add_action('user_register', 'kizumi_user_register_time');
function kizumi_user_register_time($user_id){
    $user = get_user_by('id', $user_id);
    update_user_meta($user_id, 'register_time', current_time('mysql'));
}

// 处理用户登录时间
add_action('wp_login', 'kizumi_user_login_time');
function kizumi_user_login_time($user_login){
    $user = get_user_by('login', $user_login);
    update_user_meta($user->ID, 'last_login_time', current_time('mysql'));
}

// 处理用户登录IP
add_action('wp_login', 'kizumi_user_login_ip');
function kizumi_user_login_ip($user_login){
    $user = get_user_by('login', $user_login);
