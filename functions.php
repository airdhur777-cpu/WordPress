...
// 添加图像角色脚本
function add_image_character_script() {
    // 检查是否在前台显示页面
    if (!is_admin()) {
        // 加载 JavaScript 文件
        wp_enqueue_script(
            'image-character', 
            get_template_directory_uri() . '/assets/js/girl_action.js', 
            array(), 
            '1.0.0', 
            true
        );
        ...
        // 传递配置数据给 JavaScript
        wp_localize_script('image-character', 'image_character_config', array(
            'base_url' => get_template_directory_uri() . '/assets/images/top/',
            'names' => array('lolisister1','lolisister2','dance','meow','lemon','bear'),
            'ext' => '.gif',
            'default_image' => get_kizumi('kizumi_lolijump_img') ? get_kizumi('kizumi_lolijump_img') : 'lolisister1'
        ));
    }
}
add_action('wp_enqueue_scripts', 'add_image_character_script');