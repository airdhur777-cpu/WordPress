...
function initBannerImage() {
    const bannerImg = document.querySelector('.kizumi_header_banner_img');
    const siteMain = document.querySelector('.kizumi_header_banner .site-main');
    ...
    if(img.complete && img.naturalWidth > 0) {
        ...
        img.addEventListener('load', showBanner);
        img.addEventListener('error', () => {
            // 即使图片加载失败也显示banner区域
            showBanner();
        });
        
        // 添加超时机制，确保banner一定会显示
        setTimeout(() => {
            if (!bannerImg.classList.contains('loaded')) {
                showBanner();
            }
        }, 3000);
    }
...
function initStickyHeader() {
  const header = document.querySelector('.kizumi_header .navbar');
  if (!header) return;
  let lastScrollTop = 0;
...
function initCodeCopy() {
    const container = document.querySelector('.kizumi-container');
    if (!container) return;
