...
class widget_comments extends WP_Widget {
    public function __construct() {
        parent::__construct(
            'widget_comments',
            'kizumi_最新评论', 
            array('description' => __('kizumi_最新评论侧栏', 'text_domain'),
	...
	function widget( $args, $instance ) {
		...
		foreach ( $comments as $comment ) {
            ...
            $lazy_img = kizumi_lazy_load_images();
            $comment_avatar = '<img src="'.esc_url($lazy_img).'" data-src="'.esc_url(kizumi_get_avatar_url($comment->user_id, 60)).'" class="avatar lazy" alt="avatar">';
			$output .= '<li class="comment-listitem">
                        ...
                        <div class="comment-content-link">
                          <a '.kizumi_article_new_window().' href="'.get_comment_link( $comment->comment_ID ).'" title="'.$comment->post_title.__('上的评论', 'kizumi-com').'">
	...
	function form($instance) {
		$defaults = array( 'title' => __('最新评论', 'kizumi-com'), 'limit' => 8, 'outer' => 1 );
...
?>
		...
		<p>
			<label>
				<?php echo __('排除某用户ID：', 'kizumi-com') ?>
