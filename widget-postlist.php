...
class widget_postlist extends WP_Widget {
	...
	function __construct(){
		parent::__construct( 'widget_postlist', 'kizumi_侧栏文章', array( 'description' => __('kizumi_文章侧栏', 'text_domain'),
	...
	function widget( $args, $instance ) {
		...
		echo '<article class="widget-post">
		               <div class="info">
                        <a href="'. get_the_permalink() .'" '. kizumi_article_new_window() .' class="thumb">
                          <span class="fullimage" style="background-image: url('.kizumi_article_thumbnail_src().'?'.kizumi_random_string(6).');"></span>
                        </a>
                        <div class="right">
                          <h4 class="title">
                            <a '. kizumi_article_new_window() .' href="'. get_the_permalink() .'">'. get_the_title() . get_the_subtitle() .'</a></h4>
...
?>
		...
		<p>
			<label>
				<?php echo __('排序：', 'kizumi-com') ?>
				<select style="width:100%;" id="<?php echo $this->get_field_id('orderby'); ?>" name="<?php echo $this->get_field_name('orderby'); ?>" style="width:100%;">
					<option value="comment_count" <?php selected('comment_count', $instance['orderby']); ?>><?php echo __('评论数', 'kizumi-com') ?></option>
					<option value="date" <?php selected('date', $instance['orderby']); ?>><?php echo __('发布时间', 'kizumi-com') ?></option>
					<option value="rand" <?php selected('rand', $instance['orderby']); ?>><?php echo __('随机', 'kizumi-com') ?></option>
				</select>
			</label>
		</p>
		<p>
			<label>
				<?php echo __('分类限制：', 'kizumi-com') ?>
				<a style="font-weight:bold;color:#f60;text-decoration:none;" href="javascript:;" title="<?php echo __('格式：1,2 &nbsp;表限制ID为1,2分类的文章&#13;格式：-1,-2 &nbsp;表排除分类ID为1,2的文章&#13;也可直接写1或者-1；注意逗号须是英文的', 'kizumi-com') ?>">？</a>
